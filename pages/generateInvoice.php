...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($name) && isset($price) && isset($signatureName)) {
        ...
        $qrFileName = uniqid() . '.png';
        $qrFilePath = $qrDir . $qrFileName;
        QRcode::png('https://kelasb.cerdasbelajar.xyz/2230511063/pages/admin.php', $qrFilePath);
        ...
        if ($row = $repository->findById($id)) {
            ...
            $pdf->Ln(10);
            $pdf->Text(10, $pdf->GetY(), 'Harga: ' . $productPrice);
            $pdf->Image($qrFilePath, 10, $pdf->GetY() + 20, 30);
            $pdf->Image($signatureFilePath, 150, $pdf->GetY() + 20, 40, 30);
